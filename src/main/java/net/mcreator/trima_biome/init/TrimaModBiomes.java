...
import net.mcreator.trima_biome.world.biome.TrimaBiome;
...
public class TrimaModBiomes {
	public static final DeferredRegister<Biome> REGISTRY = DeferredRegister.create(ForgeRegistries.BIOMES, TrimaMod.MODID);
	public static final RegistryObject<Biome> TRIMA = REGISTRY.register("trima", TrimaBiome::createBiome);
		...
		for (Map.Entry<ResourceKey<LevelStem>, LevelStem> entry : worldGenSettings.dimensions().entrySet()) {
			...
			if (dimensionType == dimensionTypeRegistry.getOrThrow(BuiltinDimensionTypes.OVERWORLD)) {
				...
				if (chunkGenerator.getBiomeSource() instanceof MultiNoiseBiomeSource noiseSource) {
					...
					parameters.add(new Pair<>(TrimaBiome.PARAMETER_POINT,
							biomeRegistry.getOrCreateHolderOrThrow(ResourceKey.create(Registry.BIOME_REGISTRY, TRIMA.getId()))));
				...
				if (chunkGenerator instanceof NoiseBasedChunkGenerator noiseGenerator) {
					...
					if (currentRuleSource instanceof SurfaceRules.SequenceRuleSource sequenceRuleSource) {
						...
						surfaceRules.add(1,
								preliminarySurfaceRule(ResourceKey.create(Registry.BIOME_REGISTRY, TRIMA.getId()),
										Blocks.GRASS_BLOCK.defaultBlockState(), TrimaModBlocks.TRI_GRASS.get().defaultBlockState(),
										TrimaModBlocks.TRI_LEAVES.get().defaultBlockState()));
